...
WORKDIR /usr/src
COPY /eclair/pom.xml pom.xml
COPY /eclair/eclair-core/pom.xml eclair-core/pom.xml
COPY /eclair/eclair-front/pom.xml eclair-front/pom.xml
COPY /eclair/eclair-node/pom.xml eclair-node/pom.xml
COPY /eclair/eclair-node/modules/assembly.xml eclair-node/modules/assembly.xml
...
RUN apt-get update && apt-get install -y bash jq curl tini unzip wget
